...
- **init_startup** - инициализация запуска. Сценарий лишь создан. О нем знает оркестратор(у него в бд) и апи, которая просила о его создании
- **in_startup_processing** - промежуточное состояние, олицетворяющее процесс запуска. Сценарий отправляется в сервис runner. Подготовка к инференсу, препроцессинг изображения
- **active** - активное состояние \ работа сценария. Засылаем сценарий в inference сервис. В данном статусе работает модель. Инференс сервис публикует результаты работы
